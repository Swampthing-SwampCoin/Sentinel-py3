...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, swampd):
        golist = swampd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_swampd(swampd, item)
    ...
    @classmethod
    def import_gobject_from_swampd(self, swampd, rec):
        import decimal
        import swamplib
        import inflection
        ...
        # shim/swampd conversion
        object_hex = swamplib.SHIM_deserialise_from_swampd(object_hex)
        objects = swamplib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from swampd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from swampd, with every run
        ...
        # get/create, then sync payment amounts, etc. from swampd - Swampd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(swampd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from swampd! %s" % e)
            govobj.vote_delete(swampd)
            return (govobj, None)
    ...
    def vote_delete(self, swampd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(swampd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, swampd, signal, outcome):
        import swamplib
        ...
        output = swampd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = swamplib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(swampd, signal)

    def sync_network_vote(self, swampd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = swampd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 swamp addr, non-multisig
            if not swamplib.is_valid_swamp_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Swamp address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (swamplib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SwampDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import swamplib
        obj_data = swamplib.SHIM_serialise_for_swampd(self.serialise())
    ...
    def prepare(self, swampd):
        try:
            object_hash = swampd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not swamplib.is_valid_swamp_address(addr, config.network):
    ...
    def hash(self):
        import swamplib
        return swamplib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, swampd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - swampd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, swampd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - swampd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, swampd):
        now = int(time.time())
        return (self.created_at < (now - swampd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, swampd):
        if self.is_expired(swampd):
            return False

        return True

    def is_deletable(self, swampd):
        if self.is_expired(swampd):
            return True
