...
def is_valid_swamp_address(address, network='mainnet'):
    ...
    swamp_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Swamp address
        return False

    if (address_version != swamp_version):
        return False
...
# shims 'til we can fix the swampd side
def SHIM_serialise_for_swampd(sentinel_hex):
    ...
    # superblock => "trigger" in swampd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # swampd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    swampd_hex = serialise(obj)
    return swampd_hex


# shims 'til we can fix the swampd side
def SHIM_deserialise_from_swampd(swampd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(swampd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('swamp.conf')
