...
class SwampDaemon:
    def __init__(self, **kwargs):
        ...
        # memoize calls to some swampd methods
    ...
    def from_swamp_conf(self, swamp_dot_conf):
        ...
        config_text = SwampConfig.slurp_config_file(swamp_dot_conf)
        creds = SwampConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from swamplib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = swamplib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = swamplib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = swamplib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_swampd_proto_version_with_sentinel_ping)
